...
Only stores paper trades — NO credentials stored here.
"""
...
trades_table = Table(
    ...
    Column("status",           String(20), default="OPEN"),   # OPEN / CLOSED
...
def insert_trade(trade: Dict) -> int:
    ...
    with engine.begin() as conn:
        ...
        log.info("Trade inserted id=%s", result.inserted_primary_key[0])
...
def get_open_trades() -> List[Dict]:
    with engine.connect() as conn:
        rows = conn.execute(
            trades_table.select().where(trades_table.c.status == "OPEN")
        ).fetchall()
...
def get_trades_for_date(d: date) -> List[Dict]:
    with engine.connect() as conn:
        rows = conn.execute(
            trades_table.select().where(trades_table.c.trade_date == d)
        ).fetchall()
    return [dict(r._mapping) for r in rows]
...
def get_all_trades() -> List[Dict]:
    with engine.connect() as conn:
        rows = conn.execute(
            trades_table.select().order_by(trades_table.c.entry_time.desc())
        ).fetchall()
    return [dict(r._mapping) for r in rows]
...
def get_trade_count_today() -> int:
    with engine.connect() as conn:
        row = conn.execute(
            text("SELECT COUNT(*) FROM trades WHERE trade_date = :d"),
            {"d": date.today()},
        ).fetchone()
...
def get_adjustments_for_trade(trade_id: int) -> List[Dict]:
    with engine.connect() as conn:
        rows = conn.execute(
            adjustments_table.select()
            .where(adjustments_table.c.trade_id == trade_id)
        ).fetchall()
    return [dict(r._mapping) for r in rows]
...
def get_all_daily_summaries() -> List[Dict]:
    with engine.connect() as conn:
        rows = conn.execute(
            daily_summary_table.select()
            .order_by(daily_summary_table.c.trade_date.desc())
        ).fetchall()
    return [dict(r._mapping) for r in rows]
